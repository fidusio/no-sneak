# no-sneak — Probe Configuration Reference

Reference for the `io.xlogistx.nosneak` probe framework: the JSON probe DSL, the action
...
(`/probes/https-classical.json` — a classical-only BC handshake via `mode: "classical"`, and
`/probes/smtp-starttls.json` — the STARTTLS flow recording `tls-facts` without PQC
...
```
java io.xlogistx.nosneak.ProbeChecker <host> <port> [timeoutSec] [--all|--first] [--tcp|--udp] [probe.json …]
```
...
Staged, fully non-blocking scanner in `nmap/` — embeddable (`NMapScanner.scan(NIOSocket,
...
**Output formats.** Five renderers in `nmap/output/` — `NormalFormatter`, `JSONFormatter`,
...
- **Service/version = the JSON probes** (`-sV`), strictly superior to the old banner grab → done.
...
145 pure, no-network tests in 11 classes under `src/test/java/io/xlogistx/nosneak/` (recounted
