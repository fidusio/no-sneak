 ...
 * probe forever, and since the probe never reports a result the parent fan-out
 * join barrier never reaches zero and the whole scan hangs. To prevent that, this
 * class arms its own post-connect handshake timeout via the task scheduler
